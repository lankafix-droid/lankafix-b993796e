...
const TYPE_GRADIENTS: Record<string, string> = {
  ...
  seasonal_demand: 'from-[hsl(30,90%,50%)] to-[hsl(15,85%,55%)]',
  user_recovery: 'from-[hsl(var(--lankafix-green))] to-[hsl(160,50%,40%)]',
  pending_quote: 'from-primary to-[hsl(230,60%,50%)]',
  education_info: 'from-[hsl(200,70%,45%)] to-primary',
  nearby_technicians: 'from-[hsl(var(--lankafix-green))] to-primary',
  sme_business: 'from-[hsl(220,65%,35%)] to-[hsl(240,50%,45%)]',
  warranty_assurance: 'from-primary to-[hsl(var(--lankafix-green))]',
  subscription_amc: 'from-[hsl(270,50%,45%)] to-primary',
  partner_spotlight: 'from-[hsl(var(--lankafix-green))] to-[hsl(160,60%,35%)]',
  emergency_alert: 'from-destructive to-[hsl(15,80%,50%)]',
};
...
const TrustBadgePill = ({ badge }: { badge: TrustBadge }) => (
...
const CampaignCard = memo(({ campaign, variant = 'hero', className }: CampaignCardProps) => {
  ...
  const handleClick = useCallback(() => {
    ...
    if (campaign.cta_deep_link) {
      navigate(campaign.cta_deep_link);
    }
  }, [campaign, navigate]);
  ...
  if (variant === 'compact') {
    return (
      <button
        onClick={handleClick}
        className={cn(
          'flex flex-col gap-2 rounded-2xl bg-gradient-to-br p-4 text-left text-primary-foreground shadow-md transition-all active:scale-[0.98]',
          gradient,
  ...
  return (
    ...
    >
      {/* Urgency tag */}
      {campaign.urgency_tag && (
        <span className="absolute right-4 top-4 rounded-full bg-background/25 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider backdrop-blur-sm">
          {campaign.urgency_tag}
        </span>
      )}
