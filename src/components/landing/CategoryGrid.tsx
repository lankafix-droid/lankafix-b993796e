          ...
          {categories.map((cat) => (
            <Link key={cat.code} to={`/category/${cat.code}`} className="group bg-card rounded-xl border p-5 hover:shadow-lg hover:shadow-primary/5 hover:border-primary/20 transition-all duration-300">
              <div className="flex items-start justify-between mb-3">
                <div className="w-11 h-11 rounded-lg bg-primary/10 text-primary flex items-center justify-center group-hover:bg-primary group-hover:text-primary-foreground transition-colors">{iconMap[cat.icon]}</div>
                <div className="flex gap-1 flex-wrap justify-end">
                  {cat.tags.map((tag) => (
                    <Badge key={tag} variant="outline" className={`text-xs ${tag === "Quote Required" ? "border-warning/30 text-warning" : tag === "Remote Available" ? "border-primary/30 text-primary" : "border-success/30 text-success"}`}>{tag}</Badge>
                  ))}
                </div>
              </div>
