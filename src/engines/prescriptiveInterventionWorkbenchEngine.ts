/**
 * Prescriptive Intervention Workbench Engine — Pure, deterministic.
 * Advisory-only. No side effects. No marketplace mutations.
 */

// Re-export shared types from the orchestrator to avoid duplication
export type {
  InterventionUrgency,
  ImpactLevel,
  EffortLevel,
...
export interface InterventionScoreInput {
  ...
  predictiveDecline: number;
  demandPressure: number;
  partnerDecay: number;
  overdueGovernance: number;
...
export function computePriorityInterventions(input: WorkbenchInput): PriorityInterventionsResult {
  ...
  for (const g of input.governanceRiskZones.filter(z => z.score >= 25)) {
    ...
    interventions.push({
      id: makeId("wb_gov", g.zoneId, g.categoryCode),
  ...
  for (const p of input.predictedDeclines.filter(d => d.riskLevel !== "low")) {
    ...
    interventions.push({
      id: makeId("wb_pred", p.zoneId, p.categoryCode),
  ...
  for (const pd of input.partnerDecaySignals) {
    ...
    interventions.push({
      id: makeId("wb_decay", pd.partnerId),
  ...
  for (const dp of input.demandPressureSignals.filter(d => d.riskLevel !== "low")) {
    ...
    interventions.push({
      id: makeId("wb_demand", dp.zoneId, dp.categoryCode),
  ...
  const quickWins = interventions.filter(
    i => i.estimatedEffort === "low" && (i.estimatedImpact === "medium" || i.estimatedImpact === "high") && i.priorityScore < 80
  ).slice(0, 8);
  ...
  for (const g of input.governanceRiskZones.filter(z => z.level === "CRITICAL" || z.level === "HIGH")) {
    avoidNow.push({
      id: makeId("wb_avoid_zone", g.zoneId, g.categoryCode),
